...
load_dotenv()
ENV_DEFAULTS = {
    # OpenAI API Key (Fallback, not primary for Azure)
    ...
    # Model names (used as identifiers, actual deployment names are above for Azure)
    "EMBEDDING_MODEL_IDENTIFIER": os.getenv("EMBEDDING_MODEL_IDENTIFIER", "text-embedding-ada-002"), # Identifier for UI
    "LLM_MODEL_IDENTIFIER": os.getenv("LLM_MODEL_IDENTIFIER", "gpt-4o"), # Identifier for UI
...
try:
    ...
    st.error("❌ rag_system_enhanced.py が見つかりません。アプリケーションを起動できません。") # ❌ rag_system_enhanced.py not found. Cannot launch application.
    st.stop()
       ...
       ENV_DEFAULTS["AZURE_OPENAI_CHAT_DEPLOYMENT_NAME"] and ENV_DEFAULTS["AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME"]:
        try:
            app_config = Config(
                ...
                # Other non-UI defaults or sensitive info
                db_password=os.getenv("DB_PASSWORD", "postgres"),
                # Use identifiers for model selection in UI, actual deployments are set above
            ...
            st.toast("✅ RAGシステムがAzure OpenAIで正常に初期化されました", icon="🎉") # ✅ RAG system initialized successfully with Azure OpenAI
        except Exception as e:
            st.error(f"Azure RAGシステムの初期化中にエラーが発生しました: {type(e).__name__} - {e}") # An error occurred during Azure RAG system initialization:
            st.warning("""
### 🔧 Azure OpenAI 接続エラーの解決方法 (一般的な例)
(Troubleshooting Azure OpenAI connection errors (general examples))
...
2.  **デプロイメント名**: Azureポータルで設定したチャットモデルと埋め込みモデルのデプロイメント名が正確であることを確認してください。
    (Deployment names: Ensure the deployment names for chat and embedding models set in the Azure portal are accurate.)
3.  **エンドポイントとAPIバージョン**: エンドポイントのURLとAPIバージョンが正しいか確認してください。
    (Endpoint and API version: Check if the endpoint URL and API version are correct.)
4.  **ネットワーク接続**: Azure OpenAIサービスへのネットワークアクセスが可能であることを確認してください（ファイアウォール、プロキシ設定など）。
    (Network connectivity: Ensure network access to the Azure OpenAI service is possible (firewall, proxy settings, etc.).)
            """)
            st.session_state.rag_system = None # Ensure it's None on failure
    elif ENV_DEFAULTS["OPENAI_API_KEY"]: # Fallback to OpenAI if Azure creds are not fully set
        st.warning("Azure OpenAIの完全な設定が見つかりません。OpenAI APIキーを使用してフォールバックします。") # Complete Azure OpenAI settings not found. Falling back using OpenAI API key.
        try:
            app_config = Config(
                openai_api_key=ENV_DEFAULTS["OPENAI_API_KEY"],
                db_password=os.getenv("DB_PASSWORD", "postgres"),
                llm_model_identifier=ENV_DEFAULTS["LLM_MODEL_IDENTIFIER"], # Use identifier
                embedding_model_identifier=ENV_DEFAULTS["EMBEDDING_MODEL_IDENTIFIER"], # Use identifier
                collection_name=ENV_DEFAULTS["COLLECTION_NAME"],
                final_k=ENV_DEFAULTS["FINAL_K"]
            )
            st.session_state.rag_system = initialize_rag_system(app_config)
            st.toast("✅ RAGシステムがOpenAIで正常に初期化されました", icon="🎉") # ✅ RAG system initialized successfully with OpenAI
        except Exception as e:
            st.error(f"OpenAI RAGシステムの初期化中にエラーが発生しました: {type(e).__name__} - {e}") # An error occurred during OpenAI RAG system initialization:
            # Display generic DB error help here as well, as it's a common failure point
            st.warning("""
...
(Omitted for brevity, same as original)
        ...
        st.warning("Azure OpenAIまたはOpenAIのAPIキーが設定されていません。チャット機能を利用できません。サイドバーから設定してください。")
        # OPENAI_API_KEY is not set. Chat function cannot be used. Please set it from the sidebar.
        st.session_state.rag_system = None
...
with st.sidebar:
    ...
    if rag:
        service_type = "Azure OpenAI" if rag.config.azure_openai_api_key else "OpenAI"
    ...
    with st.form("config_form"):
        st.markdown("### 🔑 Azure OpenAI API設定") # Azure OpenAI API Settings
        azure_api_key_input = st.text_input(
            "Azure OpenAI API Key",
            value=ENV_DEFAULTS["AZURE_OPENAI_API_KEY"] or "",
            type="password",
            help="Azure OpenAI APIキーを入力してください。" # Enter your Azure OpenAI API key.
        )
        azure_endpoint_input = st.text_input(
            "Azure OpenAI Endpoint",
            value=ENV_DEFAULTS["AZURE_OPENAI_ENDPOINT"] or "",
            help="Azure OpenAIのエンドポイントURLを入力してください。" # Enter your Azure OpenAI endpoint URL.
        )
        azure_api_version_input = st.text_input(
            "Azure OpenAI API Version",
            value=ENV_DEFAULTS["AZURE_OPENAI_API_VERSION"] or "2024-02-01",
            help="Azure OpenAIのAPIバージョンを入力してください (例: 2024-02-01)。" # Enter your Azure OpenAI API version (e.g., 2024-02-01).
        )
        azure_chat_deployment_input = st.text_input(
            ...
            help="Azure OpenAIのチャットモデルのデプロイメント名。" # Azure OpenAI chat model deployment name.
        )
        azure_embedding_deployment_input = st.text_input(
            ...
            help="Azure OpenAIの埋め込みモデルのデプロイメント名。" # Azure OpenAI embedding model deployment name.
        )
        st.markdown("---")
        st.markdown("### 🔑 (フォールバック) OpenAI APIキー") # (Fallback) OpenAI API Key
        openai_api_key_input = st.text_input(
            "OpenAI API Key (Fallback)",
            value=ENV_DEFAULTS["OPENAI_API_KEY"] or "",
            type="password",
            help="Azureの設定がない場合に使用されるOpenAI APIキー。" # OpenAI API key used if Azure settings are not available.
        )


        st.markdown("### 🤖 Model Identifiers (UI用)") # Model Identifiers (for UI)
        # These are just for display/selection in UI if needed, actual models are Azure deployments
        ...
        default_emb_model_id = ENV_DEFAULTS["EMBEDDING_MODEL_IDENTIFIER"]
        default_llm_model_id = ENV_DEFAULTS["LLM_MODEL_IDENTIFIER"]
        ...
        current_emb_model_id = rag.config.embedding_model_identifier if rag and hasattr(rag, 'config') else default_emb_model_id
        current_llm_model_id = rag.config.llm_model_identifier if rag and hasattr(rag, 'config') else default_llm_model_id
        ...
        # current_rrf_k = rag.config.rrf_k_for_fusion if rag and hasattr(rag, 'config') else default_rrf_k_fusion # Not in this simplified form
...
if apply_button:
    ...
    # Azure settings
    cfg_for_update.azure_openai_api_key = azure_api_key_input or ENV_DEFAULTS["AZURE_OPENAI_API_KEY"]
    cfg_for_update.azure_openai_endpoint = azure_endpoint_input or ENV_DEFAULTS["AZURE_OPENAI_ENDPOINT"]
    cfg_for_update.azure_openai_api_version = azure_api_version_input or ENV_DEFAULTS["AZURE_OPENAI_API_VERSION"]
    cfg_for_update.azure_openai_chat_deployment_name = azure_chat_deployment_input or ENV_DEFAULTS["AZURE_OPENAI_CHAT_DEPLOYMENT_NAME"]
    cfg_for_update.azure_openai_embedding_deployment_name = azure_embedding_deployment_input or ENV_DEFAULTS["AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME"]
    
    # Fallback OpenAI API Key
    cfg_for_update.openai_api_key = openai_api_key_input or ENV_DEFAULTS["OPENAI_API_KEY"]

    # Model Identifiers
    cfg_for_update.embedding_model_identifier = embedding_model_id_sb
    cfg_for_update.llm_model_identifier = llm_model_id_sb
...
with tab_chat: # tab1 から tab_chat に変更 (Change from tab1 to tab_chat)
    if not rag:
        st.info("🔧 RAGシステムが初期化されていません。サイドバーでAzure OpenAIまたはOpenAI APIキーを設定し、「Apply Settings」をクリックするか、データベース設定を確認してください。") # 🔧 RAG system is not initialized. Set Azure OpenAI or OpenAI API key in the sidebar and click "Apply Settings", or check database settings.
    else:
...
with tab_settings: # tab4 から tab_settings に変更 (Change from tab4 to tab_settings)
    ...
    with st.form("detailed_settings_form_v7_tab_settings"):
        ...
        with col1_settings:
            st.markdown("#### 🔑 Azure OpenAI 設定") # Azure OpenAI Settings
            ...
            st.markdown("---")
            st.markdown("#### 🔑 (フォールバック) OpenAI APIキー設定") # (Fallback) OpenAI API Key Settings
            form_openai_api_key = st.text_input(
                "OpenAI APIキー (Fallback)", 
                value=current_values_dict.get("openai_api_key", "") or "", 
                type="password",
                key="setting_openai_key_v7_tab_settings",
                help="Azureの設定がない場合に使用されるOpenAI APIキー。"
            )

            st.markdown("#### 🤖 AIモデル識別子 (UI用)") # AI Model Identifiers (for UI)
            ...
            embedding_model_id_form_val = st.selectbox("埋め込みモデル識別子", emb_opts_form, index=emb_idx_form, help="埋め込みモデルのUI表示用識別子", key="setting_emb_model_id_v7") # Embedding model identifier (UI display identifier for embedding model)
            ...
            llm_model_id_form_val = st.selectbox("言語モデル識別子", llm_opts_form, index=llm_idx_form, help="言語モデルのUI表示用識別子", key="setting_llm_model_id_v7") # Language model identifier (UI display identifier for language model)
        ...
        with col2_settings:
            ...
            # For Azure, embedding dimensions are often tied to the deployment, so direct user input might be less critical here.
            # If needed, it can be re-added. For now, it's commented out to simplify Azure config.
    ...
    if apply_settings_button_form:
        try:
            new_app_config_obj = Config( # Renamed variable
                # Azure OpenAI settings from form
                ...
                # Fallback OpenAI API Key
                openai_api_key=form_openai_api_key,
                # Model Identifiers
                ...
                # embedding_dimensions=embedding_dimensions_form_val, # Commented out
            ...
            if rag and hasattr(rag, 'config'):
                if new_app_config_obj != rag.config: 
    ...
    if reset_settings_button_form:
        ...
        # Ensure default Azure and OpenAI keys from ENV_DEFAULTS are used for reset
        default_config_for_reset_obj = Config(
            ...
            azure_openai_chat_deployment_name=ENV_DEFAULTS["AZURE_OPENAI_CHAT_DEPLOYMENT_NAME"],
            azure_openai_embedding_deployment_name=ENV_DEFAULTS["AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME"],
            openai_api_key=ENV_DEFAULTS["OPENAI_API_KEY"] # Also reset fallback
        )
